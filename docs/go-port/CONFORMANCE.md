...
| Coding-agent app-server subprocess client with JSON line protocol | Section 17.5 | `internal/agent/codex` | `implemented` | `TOO-126` adds a runner-facing Codex app-server JSONL client using the Codex app-server protocol shape for `initialize`, `thread/start`, `turn/start`, notifications, token usage, and terminal/error events. Fake app-server helper-process tests cover success, malformed JSON, read timeout, stall timeout, turn timeout, process error, subprocess cwd/env behavior, approval/sandbox payload forwarding, live event callbacks, and both legacy `item/tool/call` plus Codex 0.130 `mcpServer/tool/call` tool request shapes. Real Codex credential dogfood remains deferred to the real integration profile. |
...
| Strict prompt rendering with `issue` and `attempt` variables | Section 17.1, 12 | `internal/agent`, `internal/orchestrator` | `implemented` | `TOO-127` adds the orchestrator-facing `internal/agent` runner with strict Liquid-style dotted interpolation for normalized `issue` fields and optional `attempt`, render failures for missing variables / unknown filters before Codex launch, max-turn loop enforcement, continuation prompt handling, Codex timeout/cwd request wiring, and normalized run/session metadata. `TOO-150` exposes `issue.comments` as bounded JSON so the default prompt can include threaded Linear discussion without replacing the Workpad protocol. Fake turn-client tests cover prompt rendering, missing variables, unknown filters, first/retry attempt behavior, cwd, timeout propagation, max turns, discussion context, and metadata; orchestrator tests cover issue/attempt/config wiring into the runner. Full Liquid control-flow tags and real Codex dogfood remain deferred to later profiles. |
| Exponential retry queue with explicit continuation and failure retries | Section 17.4, 12.3 | `internal/orchestrator`, `internal/state` | `implemented` | `TOO-125` adds retry entries with issue id, identifier, attempt, due time, and error; explicit in-run continuation still happens through `TurnResult.Continue` and `max_turns`, while normal worker completion is now a local terminal outcome instead of an automatic retry. Retryable abnormal exit schedules 10s-based exponential retries; due retry handling re-fetches active candidates, dispatches eligible retries, requeues slot exhaustion, and releases missing/non-eligible candidates. `TOO-132` persists retry rows in SQLite when `state_store.path` is enabled, deletes released/claimed retry rows, and fake-tests restart recovery of persisted retries. |
...
| Structured logs with `issue_id`, `issue_identifier`, and `session_id` | Section 17.6 | `internal/observability`, `internal/orchestrator` | `implemented` | `TOO-129` adds shared observability event types, JSON logger, discard logger, deterministic recorder, and orchestrator wiring for dispatch, workspace, hook, agent, retry, reconcile, tracker-error, cleanup events, and live Codex app-server event persistence. Focused fake tests assert emitted issue identifiers, session id, run status, retry/suppression state, token usage, and error fields. |
...
| Persist retry queue and session metadata across process restarts | Recommended production readiness | Selected for production baseline | `implemented` | `TOO-132` adds optional `state_store.path` config, `internal/state` SQLite migrations, durable `runs` / `retry_queue` / `issue_suppressions` / `sessions` / `agent_events` tables, orchestrator Store interface integration, startup recovery, focused SQLite tests for missing/corrupt DB and idempotent recovery, and orchestrator tests proving persisted retry/interrupted-run recovery plus restart-safe local-terminal suppression. |
...
| Add first-class tracker write APIs for comments/state transitions instead of only agent tools | Recommended extension | Selected for Linear workpad/state transition reliability | `implemented` | `TOO-135` adds typed Linear write APIs in `internal/tracker/linear` for issue comment create/update, idempotent workpad upsert by heading, issue state transition by state ID or team state name lookup, and URL attachment links. `TOO-150` adds typed parent-comment reply creation through `commentCreate(parentId)` and records the supporting Linear schema introspection. Fake GraphQL server tests cover success paths, duplicate workpad heading handling without creating another active comment, missing issue/comment semantics, auth/status/GraphQL errors, state lookup failure, reply missing-field/unknown-payload handling, and link handling. The raw `linear_graphql` tool remains available as an escape hatch. |
...
| Safe sandbox defaults and scoped credentials | Architecture Section 8 | `implemented` | `TOO-134` changes `WORKFLOW.md` and config defaults to `approval_policy: on-request`, `thread_sandbox: workspace-write`, and `turn_sandbox_policy.type: workspaceWrite`; validation rejects `never`, `danger-full-access` / `dangerFullAccess`, and all-shell environment inheritance unless the operator explicitly starts a trusted local run with `--allow-unsafe-codex` or `SYMPHONY_ALLOW_UNSAFE_CODEX=true`. This is a production baseline posture and does not claim enterprise RBAC, approval queues, container isolation, or a secret-manager provider. |
| Secret redaction, audit log, sandbox/cost guardrails | Architecture Sections 8, 15 | `implemented` | `TOO-134` adds centralized redaction in `internal/safety`, applies it before runtime logger and durable audit writes, persists redacted hook/agent/tool/guardrail events through the existing state store event surface, and enforces per-issue max-turn, max-duration, opt-in positive max-token, and configured estimated-cost guardrails. |
...
| Per-project concurrency, per-issue timeout, max turns, and cost limits | Architecture Sections 6.2, 8, 13 | `implemented` | Earlier orchestrator slices enforce global and per-state concurrency. `TOO-134` adds per-issue max turns, max run duration, opt-in positive max total tokens, and optional estimated token-cost limits, with configured guardrail stops treated as non-retryable resource boundary events. |
| Typed Linear writes for comments and state transitions | Architecture Section 6.2 | `implemented` | `TOO-135` adds first-class Linear write APIs for comment create/update, single-heading workpad upsert, state transition, and URL attachment link writeback, with deterministic fake server coverage and no browser-based workflow. `TOO-150` extends this surface with parent comment replies plus bounded threaded discussion read context. |
...
| Approval, sandbox, and user-input policy | `implementation_defined` | `TOO-134` chooses a production baseline of non-`never` approval plus workspace-write sandbox defaults. Config validation rejects the known unsafe local posture values by default rather than silently passing them through; trusted local operators can opt in explicitly with `--allow-unsafe-codex` or `SYMPHONY_ALLOW_UNSAFE_CODEX=true`. Full approval queue semantics remain out of scope. |
| Codex config validation depth | `implementation_defined` | `TOO-134` keeps Codex config mostly pass-through, but adds explicit production-baseline rejection for `approval_policy: never`, `thread_sandbox: danger-full-access`, `turn_sandbox_policy.type: dangerFullAccess`, and all-shell environment inheritance unless the unsafe Codex opt-in is set. The port still does not maintain a full Codex enum catalog. |
