...
| Exponential retry queue with continuation retries after normal exit | Section 17.4, 12.3 | `internal/orchestrator`, `internal/state` | `implemented` | `TOO-125` adds in-memory `state.Retry` entries with issue id, identifier, attempt, due time, and error; normal worker exit schedules a 1s continuation retry, abnormal exit schedules 10s-based exponential retries, due retry handling re-fetches active candidates, dispatches eligible retries, requeues slot exhaustion, and releases missing/non-eligible candidates. Deterministic fake-clock tests cover continuation dispatch, retry release, and worker-error retries. Durable restart persistence remains deferred. |
...
| Operator-visible observability (structured logs; optional snapshot/status surface) | Section 17.6, 13 | `internal/observability`, `internal/orchestrator` | `implemented` | `TOO-129` adds `Runtime.Snapshot()` as a read-only status snapshot covering lifecycle state, active runs, and retry queue rows. Focused fake-clock tests verify active-run and retry snapshot consistency. HTTP status endpoints, Prometheus metrics, durable audit log, and CLI `status` behavior remain deferred to `TOO-133` / later production slices. |
...
| --- | --- | --- | --- |
| Run the Real Integration Profile from Section 17.8 with valid credentials and network access | Phase 3 Go Dogfood, `TOO-131` | `deferred` | `TOO-126` provides fake deterministic Codex protocol coverage; real Linear/Codex dogfood remains intentionally deferred to `TOO-131` and is not part of the default CI profile. |
...
| Orphan workspace reconciliation and terminal cleanup hardening | Architecture Sections 7, 14 | `partial` | `TOO-125` implements tracker-driven startup terminal cleanup and active terminal transition cleanup for known terminal issues. Filesystem orphan discovery and durable recovery remain for `TOO-132`. |
