...
| --- | --- | --- | --- | --- |
| Workflow path selection supports explicit runtime path and cwd default | Section 17.1, 17.7 | `cmd/symphony`, `internal/workflow` | `partial` | CLI path resolution and readable-file check exist; deterministic tests and full runtime start behavior remain for `TOO-116`. |
...
| Hook timeout config (`hooks.timeout_ms`, default `60000`) | Section 17.2 | `internal/config`, `internal/hooks` | `partial` | Config field exists; defaulting and enforcement remain for `TOO-119` / `TOO-121`. |
...
| Codex launch command config (`codex.command`, default `codex app-server`) | Section 17.1, 17.5 | `internal/config`, `internal/agent/codex` | `partial` | Config field exists; defaulting and launch behavior remain for `TOO-119` / `TOO-126`. |
...
| Configurable retry backoff cap (`agent.max_retry_backoff_ms`, default 5m) | Section 17.4 | `internal/config`, `internal/orchestrator` | `not_started` | The current config struct does not yet expose this field; planned for `TOO-125`. |
...
| Workflow validation command | Architecture Sections 6.2, 13 | `partial` | CLI command exists but returns not implemented; planned across `TOO-116`, `TOO-117`, and `TOO-119`. |
