...
| `DOCS-001` | `docs` | issue / PR closeout 必须回写 `ChangeLog.md` 的 `Unreleased`，并在结果摘要记录 `changelog_action` 与 `changelog_version` | `.agents/skills/symphony-go-version-release/SKILL.md` | harness gate | `make harness-check` | `enforced` | `maintenance_candidate` | gate 会检查当前分支或工作区变更是否包含 `ChangeLog.md` / `changeLog.md` |
